...
async function updateMashup() {
    console.log(`Updating Mashup: ${MASHUP_NAME}...`);

    // Build the correct mashupContent structure based on the working example
    const mashupContent = {
        "UI": {
            "Properties": {
                "Area": "Mashup",
                    ...
                    "Widgets": [
                        {
                            "Properties": {
                                ...
                                "ShowDataLoading": true,
                                "Visible": true,
                                "Top": 286.4583282470703,
                                "Left": 639.0277709960938,
                                "Z-index": 10,
                                "Id": "ptcslabel-3",
    ...
    const payload = {
        ...
        "description": "Test mashup created by Antigravity Agent - FIXED",
    ...
    const url = `${BASE_URL}/Mashups?Content-Type=application%2Fjson&reason=fixed%20by%20Antigravity`;
    ...
    if (!response.ok) {
        ...
        console.error(`Failed to update mashup: ${response.status} ${response.statusText}`);
        console.error(text);
    } else {
        console.log(`✓ Mashup '${MASHUP_NAME}' updated successfully with correct structure!`);
